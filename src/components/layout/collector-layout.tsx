...
export function CollectorLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: 'Dashboard', href: '/collector', icon: LayoutDashboard },
  ];
      ...
      <aside className="hidden md:flex flex-col w-64 border-r bg-white min-h-screen sticky top-0">
        <div className="p-6 border-b">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-[10px] uppercase font-bold tracking-wider bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full ml-1">Col</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-xs text-muted-foreground capitalize">{currentUser.role} • {currentUser.area}</p>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1">
        ...
        <header className="md:hidden bg-white border-b p-4 flex items-center justify-between sticky top-0 z-20 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg tracking-tight">EcoLoop Collector</span>
          </div>
