...
import { Recycle, LogOut, Package } from "lucide-react";
...
export function RecyclerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: 'Materials', href: '/recycler', icon: Package },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row pb-16 md:pb-0">
      ...
      <aside className="hidden md:flex flex-col w-64 border-r bg-white min-h-screen sticky top-0">
        <div className="p-6 border-b">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-[10px] uppercase font-bold tracking-wider bg-green-100 text-green-700 px-2 py-0.5 rounded-full ml-1">Recycle</span>
          </div>
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map((item) => {
            const isActive = pathname === item.href || (item.href !== '/recycler' && pathname.startsWith(item.href));
            return (
              <Link 
                key={item.href} 
                href={item.href}
        ...
        <div className="p-4 border-t">
          <button 
            ...
            className="flex w-full items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-muted-foreground hover:bg-slate-50 hover:text-foreground transition-colors"
          >
      ...
      <main className="flex-1">
        {/* Mobile Header */}
        <header className="md:hidden bg-white border-b p-4 flex items-center justify-between sticky top-0 z-20 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg tracking-tight">EcoLoop Recycler</span>
          </div>
