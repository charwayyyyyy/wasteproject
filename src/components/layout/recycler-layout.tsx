...
export function RecyclerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: 'Market', href: '/recycler', icon: Recycle },
    { name: 'My Claims', href: '/recycler/claims', icon: Package },
    { name: 'History', href: '/recycler/history', icon: CheckSquare },
  ];
      ...
      <div className="min-h-screen bg-background-secondary flex flex-col md:flex-row pb-[env(safe-area-inset-bottom)]">
        ...
        <aside className="hidden md:flex flex-col w-[260px] border-r border-border-subtle bg-surface min-h-screen sticky top-0 shadow-[1px_0_10px_rgba(0,0,0,0.02)] z-30">
          <div className="p-6 border-b border-border-subtle flex items-center justify-between">
            <Link href="/recycler" className="flex items-center gap-2">
          ...
          <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto no-scrollbar">
            <div className="px-3 pb-2 pt-1 text-xs font-semibold text-text-tertiary uppercase tracking-wider">Recycling Partner</div>
            {navItems.map((item) => {
          ...
          <div className="p-4 border-t border-border-subtle bg-surface/50 backdrop-blur-md">
            <div className="flex items-center gap-3 mb-4 px-3">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-text-tertiary capitalize truncate">Verified Partner</p>
              </div>
            </div>
            <button 
        ...
        <main className="flex-1 flex flex-col min-w-0 pb-[88px] md:pb-0">
          ...
          <header className="md:hidden bg-surface/80 backdrop-blur-xl border-b border-border-subtle p-4 flex items-center justify-between sticky top-0 z-20 pt-[max(env(safe-area-inset-top),16px)]">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-[17px] tracking-tight text-text-primary">EcoLoop Recycler</span>
            </div>
